...
- [x] **`AccountFile_Redistribution` 대상 storage row에 `SELECT ... FOR UPDATE`** — **무효화됨(전제 붕괴), 사유 아래**. 이 항목은 진단 당시 "재분배가 서버 프로세스 안에서(다운로드→업로드→기록까지) 통째로 일어난다"는 전제로 쓰였다. Phase 4에서 스토리지 삭제/재분배를 파일 업/다운로드와 동일한 패턴(서버는 계획만 수립, 실제 다운로드→업로드는 클라이언트가 직접 수행, 완료 후 서버에 보고)으로 재설계하면서 그 전제 자체가 사라졌다 — 서버가 몇 초~몇십 초 걸리는 클라이언트 전송 시간 동안 DB 트랜잭션 락을 들고 있을 수는 없다.
  - **예약(reservation) 방식은 기각**: 계획 수립 시 짧게 락을 걸고 용량을 미리 반영해두는 방식도 검토했으나, 클라이언트 전송이 완료 보고 없이 끝나는 경우(크래시, 네트워크 끊김, 앱 강제 종료) 예약분을 되돌릴 방법이 없어 TTL/reaper 같은 만료·정리 인프라가 별도로 필요하다. 지금 프로젝트엔 그런 백그라운드 정리 체계가 전혀 없고, 이번 재설계 하나를 위해 새로 만들 정도의 스코프도 아니다.
  - **결정**: 재분배도 이미 정상 업로드 경로(`select-storage`/`confirm-upload`)가 감수하고 있는 것과 동일한 수준의 TOCTOU를 그대로 받아들인다 — 계획 조회는 순수 조회(락 없음), 완료 보고 시 `IncrementUsedCapacity`로 blind update(용량 상한 체크 없이 `used_capacity + delta`만 반영, 지금 코드와 동일). 계획 시점과 완료 보고 시점 사이에 다른 요청이 같은 target storage를 노려 아주 드물게 소폭 초과가 생길 수 있지만, 이는 프로젝트 전역에서 이미 받아들이고 있는 리스크 수준이라 재분배만 따로 더 엄격하게 다룰 이유가 없다.
  - 파일 단위 부분 실패(일부 파일만 이동)나 업로드 중단 시 롤백 미구현 같은 나머지 문제는 여전히 알려진 한계로 남는다.
...
- [ ] **Phase 4 — 클라이언트 리팩토링**: `DbConfig.cs`/직접 DB 접속 코드 제거, `LanTransferService`의 DB 직접 조회를 `presence` API 호출로 교체, API 클라이언트로 교체, 로그인/토큰 발급 흐름 변경
  ...
  - [x] coop/이슈 클라이언트 교체 — `CoopUserRepository`/`CooperationManager`/`FileIssueRepository`/`FileIssueCommentRepository`/`FileIssueMappingRepository`를 직접 호출하던 14개 View 전부를 위 `/api/coop/*`·`/api/issues/*`로 교체(폴백 없음 — 파일 업/다운로드와 같은 기준, presence/로그인과 다름). `OverCloudApiClient`에 coop 5개 + issue 8개 메서드 추가, 응답은 기존 `DB.overcloud.Models.FileIssueInfo`/`FileIssueComment`를 `PropertyNameCaseInsensitive`로 그대로 역직렬화해 View의 기존 바인딩을 유지:
    - **본인 계정 케이스** 앵커: `LoginWindow`(로그인 시 협업 계정 목록 구성) — API 실패해도 로그인 자체는 막지 않고 개인 계정만으로 진행(LoginAsync와 동일 원칙 유지).
    - **협업 계정 케이스** 앵커: `SharedAccountView.Button_CreateIssue_Click` — `coopId`(`_currentAccountId`)를 명시적으로 넘겨야 하는 지점이라, 예전 업로드 버그(엉뚱한 계정으로 새는 것)와 같은 클래스의 실수가 재발하지 않는지 특히 신경 씀.
    - `SharedAccountView`의 `LoadAccountTrees`/`RefreshExplorer`/`LoadFolderContents`는 각각 3곳/9곳/6곳(라이브 호출부 기준, 주석 처리된 죽은 코드 제외)에서 호출되고 있어 async 전파가 필요했다 — 전부 async로 바꾸고 호출부(대부분 이미 async void였던 이벤트 핸들러, 일부는 `async void`로 신규 전환)까지 정리. `RightFileItem_Click`의 `Task.Run(() => Dispatcher.Invoke(...))`는 이미 UI 스레드에서 도는 이벤트 핸들러 안의 불필요한 우회였고 애초에 그 안에서는 await를 쓸 수 없어 제거 — 그냥 직접 `await`.
    - `LoadFolderContents`(파일 목록 + 파일별 이슈 상태 표시)는 파일 개수만큼 순차 호출하면 느려서 `Task.WhenAll`로 병렬 조회하도록 재구성.
    - `CreatedBy`/`CommenterId`/coop 생성자는 더 이상 클라이언트가 값을 안 보낸다 — 서버가 토큰의 `sub`로 강제(위조 방지, 이미 서버 쪽에 구현됨).
    - fileId→전체 경로 해석(`GetFullPath`)은 이번 범위 밖이라 기존 DB 직접 접속 그대로 둠.
  - [x] 스토리지 추가/삭제 순차 추가
    ...
    - [x] **스토리지 삭제(`Delete_Cloud_Storage`) 재분배 — API 서버 + 클라이언트 오케스트레이션 구현 완료**. `select-storage`/`confirm-upload`와 동일한 3단계 계약으로 설계:
      - `GET /api/storages/{cloudStorageNum}/redistribution-plan?userId=` — 이동 대상 파일마다 후보 스토리지를 랭킹 리스트(최대 5개, `GetCandidateStorages`는 원래 상한이 없어서 여기서 캡을 둠)로 내려줌. 사전 용량 체크(`GetTotalRemainingQuotaInBytes_Delete_Account` vs `AllFilelistSize`)도 여기서 함 — **버그 수정**: 원래 `AccountService.Delete_Cloud_Storage`가 바이트(전자)와 KB(후자)를 단위 변환 없이 그냥 비교하고 있어서 사실상 항상 통과하는 무의미한 체크였다. 여기서 `AllFilelistSize(...) * 1024`로 바이트로 맞춰서 비교하도록 고침.
      - `POST /api/files/{fileId}/relocate-confirm` — 클라이언트가 새 클라우드 업로드 + 옛 클라우드 삭제까지 마친 뒤 호출. `CloudFileInfo` 갱신 + 옛/새 스토리지 `used_capacity` 양쪽 blind update.
      - `DELETE /api/storages/{cloudStorageNum}?userId=` — 남은 파일이 있으면 막음. 실제로 삭제 진행.
      - **분산 파일(조각) 처리 — 일반 파일과 동일 메커니즘으로 지원됨**: 초안에서는 "완전히 다른 설계가 필요하다"고 보고 `RootFileId.HasValue`인 행(조각)을 `unsupported: true`로 표시해 삭제 자체를 막았으나(안내 메시지로 "다운로드 후 재업로드" 유도), 이 전제가 틀렸음이 확인돼 철회함 — 조각은 업로드 시점에 이미 "한 클라우드에 통째로 들어가는 크기"로 쪼개진 상태라, 그 조각 하나를 다른 클라우드로 옮기는 데 새로운 재분할 로직이 필요 없다. 일반 파일과 완전히 동일하게 `GetCandidateStorages(조각.FileSize, ...)`로 후보를 계산하고 `relocate-confirm`으로 옮긴다 — `updateFile`이 `cloud_storage_num`/`cloud_file_id`만 갱신하고 `root_file_id`/`chunk_index`는 그대로 두므로 논리 파일-조각 관계가 깨지지 않고, `FileDownloadManager.DownloadAndMergeFile`도 조각마다 그때그때 `CloudStorageNum`을 조회해 병합하므로 조각이 재배치돼도 다운로드에 투명하다. `RedistributionFilePlan`/`RedistributionPlan`에서 `Unsupported`/`HasUnsupportedFiles` 필드 제거, `DELETE` 엔드포인트의 조각 전용 차단 분기도 제거(남은 파일이 있으면 막는 일반 체크 하나로 통합). **주의**: `IsDistributed` 플래그는 논리 파일 행에만 세팅되고 조각 행엔 안 세팅돼 있어서, 조각 여부 판별이 필요한 다른 코드(있다면)에서 이 플래그를 쓰면 안 되고 `RootFileId.HasValue`를 써야 한다.
      - **검증: 분산 재분배 특유의 두 상황도 문제없음을 확인함**:
        - *조각별 서로 다른 클라우드 배치*: 원래 `Upload_Distributed`엔 "형제 조각은 같은 클라우드에 못 들어간다"는 명시적 규칙이 없었다 — `GetStoragePlan`이 연결된 계정 목록(중복 없음)을 한 번씩만 순회해 `selected`를 만들고, `Upload_Distributed`가 그 리스트를 그대로 순회해 조각을 만드는 구조상 부수적으로 "조각마다 다른 클라우드"가 됐을 뿐이다. `GetCandidateStorages`(재분배 후보 계산)는 이 부수 효과를 전혀 모르고 삭제 대상 스토리지만 제외한 채 여유 공간 기준으로만 후보를 고른다 — 그래서 재분배 후 형제 조각 두 개가 같은 클라우드로 몰릴 수 있다. **이건 의도적으로 허용함**: 재분배는 클라우드 하나가 풀에서 빠지는 상황이라, 예를 들어 연결된 클라우드가 2개뿐이고 그중 하나를 지우면 남은 후보가 1개뿐이라 형제 조각과 겹치는 걸 피할 방법이 원천적으로 없다 — 여기에 "형제 조각과 겹치지 말 것" 제약을 걸면 이런 흔한 축소 시나리오에서 오히려 재분배 자체가 불가능해진다.
        - *같은 스토리지의 형제 조각 2개 이상*: `GetFilesByStorageNum`은 단순 `WHERE cloud_storage_num = @num` 조회라 형제 조각도 서로 다른 `FileId`/`ChunkIndex`/`CloudFileId`를 가진 독립된 행으로 반환되고, 클라이언트도 `foreach`로 순차 처리하며 파일마다 별도 임시 경로/토큰 조회를 쓰므로 공유 상태가 없어 순서 문제가 없다. 유일하게 있을 수 있는 건 계획 조회 시점 스냅샷이 오래돼(첫 조각이 채운 뒤 둘째 조각이 같은 1순위 후보를 시도) 실패할 가능성인데, 이건 새로운 문제가 아니라 이미 감수하기로 한 blind-update TOCTOU이고 실패 시 다음 순위 후보로 자동 폴백하는 기존 랭킹 리스트 메커니즘이 그대로 커버한다.
      - **부분 실패 재시도**: 별도 로직 없이 이미 지원됨 — `relocate-confirm` 성공 시 그 파일의 `CloudStorageNum`이 바뀌므로, `redistribution-plan`을 다시 호출하면 `GetFilesByStorageNum`이 자동으로 남은 파일만 반환한다.
      - **락/예약 없음(5.7 결정)**: 재분배가 서버 프로세스 안에서 통째로 일어난다는 원래 5.7 TODO의 전제가 이 재설계로 무효화됨 — 예약 방식은 TTL/reaper 인프라가 필요해 스코프 초과로 기각, 대신 `select-storage`/`confirm-upload`와 동일한 수준의 blind-update TOCTOU를 그대로 받아들이기로 함(5.7 항목에 상세 기록).
      - **클라이언트 오케스트레이션**: `UI/overcloud/overcloud/transfer_manager/StorageRedistributionManager.cs`(신규) — 계획을 받아 파일마다 옛 클라우드에서 다운로드 → 후보를 순서대로 시도해 업로드(첫 성공에서 종료, 실패하면 서버 재호출 없이 다음 후보로) → `relocate-confirm` → 옛 클라우드에서 실제 삭제(베스트 에포트, 실패해도 메타데이터는 이미 옮겨졌으니 재분배 자체는 성공 처리). `relocate-confirm` 실패 시 방금 올린 새 클라우드 파일을 정리 시도. 기존 `GoogleDriveTokenClient`/`OneDriveTokenClient`(`DownloadAsync`/`UploadAsync`/`DeleteAsync`)와 `/api/oauth/{provider}/access-token`(cloudStorageNum 기반이라 old/new 둘 다 재사용)만으로 충분, 새 OAuth 엔드포인트는 필요 없었음. 계획 응답에 `CloudFileId`(다운로드용) 필드 추가 필요했음(초안엔 누락).
      - **버그 수정**: `DeleteAccountWindow.ConfirmDelete_Click`이 삭제 대상 스토리지의 소유자로 항상 `_user_id`(로그인한 본인)를 넘기고 있었다 — 협업 계정(`_is_shared=true`) 스토리지를 지울 때는 그 협업 계정 ID를 넘겨야 하는데, 실제로는 `selectedAccount.ID`(목록에 표시된 계정이 실제로 속한 소유자)를 써야 했다. 기존 코드로는 협업 계정 스토리지 삭제가 서버 조회(`GetCloud(cloudStorageNum, userId)`) 단계에서 항상 실패했을 것으로 보임 — 이번에 다시 짜면서 `selectedAccount.ID`로 고침.
      - 연결된 `AccountService.Delete_Cloud_Storage`/`AccountFile_Redistribution`(DB 직접) 클라이언트 호출부는 이제 없음(grep 확인) — 서버 쪽 레거시 코드 자체는 안 지움(다른 참조 여부 미확인, 범위 밖).
    - [x] **스토리지 추가(`Add_Cloud_Storage`) 재설계 — 인터랙티브 OAuth를 API 기반으로 완료**. 로컬 `C:\key\credential.json`(Google client_secret 포함)/`onedrive_credential.json` 의존을 완전히 제거:
      - `GoogleOAuthService`/`OneDriveOAuthService`에 `ExchangeAuthorizationCodeAsync` 추가(기존 `ExchangeRefreshTokenAsync`와 같은 클래스, 서버 설정의 client_secret만 씀). `RedirectUri`/`Scope`를 `private const`에서 `public const`로 바꿔 새 엔드포인트에서 재사용.
      - 신규 엔드포인트 3개: `GET /api/oauth/{provider}/client-config`(client_id/redirect_uri/scope — 비밀 아님, 클라이언트가 로컬 파일 대신 여기서 받음), `POST /api/oauth/{provider}/exchange-code`(authorization code → access/refresh token, client_secret은 서버 밖으로 안 나감), `POST /api/storages`(계정 연동 완료 후 `CloudStorageInfo` 행 생성 — select-storage/confirm-upload와 동일하게 "클라이언트가 끝낸 뒤 서버는 메타데이터만" 계약).
      - **원칙 유지**: 이메일/용량 조회(`about.get`/`GET /me/drive`)는 서버가 아니라 클라이언트가 방금 받은 access token으로 직접 함 — "서버는 클라우드 제공자에 직접 접속하지 않는다"(파일 이관 때 세운 원칙)를 스토리지 추가에도 그대로 적용. `GoogleDriveTokenClient`/`OneDriveTokenClient`에 `GetAccountInfoAsync` 추가(기존 `GoogleDriveService`/`OneDriveService`의 이메일/용량 조회 로직을 토큰 전용으로 포팅).
      - **`ClientId`/`ClientSecret` 컬럼은 서버가 채움**: 아직 안 옮겨진 분산 저장 경로(`FileUploadManager.Upload_Distributed` 등)가 `CloudStorageInfo.ClientId`/`ClientSecret`을 DB에서 직접 읽어 자체 refresh하고 있어서, 새로 추가되는 계정도 이 컬럼이 채워져 있어야 그 기능이 계속 동작한다 — `/api/storages`가 클라이언트가 보낸 값이 아니라 서버 설정 그대로 채워 저장.
      - **클라이언트 오케스트레이션**: `UI/overcloud/overcloud/CloudApi/OAuthRedirectListener.cs`(신규, 로컬 리다이렉트에서 code 캡처하는 Google/OneDrive 공통 헬퍼), `UI/overcloud/overcloud/transfer_manager/StorageAddManager.cs`(신규, client-config→브라우저 열기→code 캡처→exchange-code→이메일/용량 조회→`/api/storages` 전체 오케스트레이션). `StorageRedistributionManager`와 동일한 이유로 `AccountService`(로그인 등 다른 곳에서도 쓰이는 공유 클래스)를 직접 고치지 않고 새 클래스로 분리 — 영향 범위를 좁히고 나중에 옛 코드를 안전하게 통째로 지울 수 있게.
      - `AddAccountWindow.xaml.cs`: GoogleDrive/OneDrive는 `StorageAddManager`로, Dropbox는 기존 `AccountService.Add_Cloud_Storage`(DB 직접) 그대로 유지(Dropbox 자체 이관은 범위 밖). 실패 시 창이 안 닫히도록 같이 고침.
      - **버그 수정**: `AccountService.Add_Cloud_Storage`가 `AddCloudStorage` 성공 후 방금 추가한 계정을 `GetAllAccounts`로 다시 찾아서 그 결과(`OneCloud`)를 바로 역참조하고 있었는데, 못 찾는 경우(null) 방어가 없어 `NullReferenceException` 위험이 있었다 — null 체크 추가. GoogleDrive/OneDrive 분기는 이제 `AddAccountWindow`에서 호출되지 않아 사실상 Dropbox 전용 경로로만 남음(자체는 안 지움 — `Delete_Cloud_Storage`/`AccountFile_Redistribution` 때와 동일한 처리).
      - **PKCE 미적용**: 기존 두 플로우 다 PKCE 없이 동작 중이었고 이번 재설계는 "재배포 가능하게 만들기"가 목적이라 동일 수준 유지 — 필요하면 별도 하드닝으로 나중에.
